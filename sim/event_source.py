#!/usr/bin/env python3
# Port of CANavigator's tools/event_emitter.py::EventEmitter: was an
# rclpy.Node polling a ROS /clock-derived sim time on a background thread and
# publishing std_msgs/String; here it's a plain class whose step() is called
# once per sim tick from the main drive loop (consumer: sim.sim_adapters.
# EventQueue) -- no thread, no ROS topic. Deterministic mode (the only mode
# experiment/orchestrator.py uses) is a pure logical clock driven by SimClock
# ticks, needing no wall-clock/pose input.
from __future__ import annotations
from dataclasses import dataclass, field
from typing import Any, Dict, Optional
import json, math, random, csv, os

from sim.sim_adapters import SimClock, EventQueue
...
class EventEmitter:
    ...
    RNG-based from `seed`, driven by a logical clock advanced via SimClock.

    Non-deterministic mode: uses sim-time deltas; push_pose() must be called
    each tick, though pose isn't actually read anywhere (kept for API
    ...
    def __init__(self, events_out: EventQueue, sim_clock: SimClock, gen_cfg: Optional[EventCfg] = None):
        ...
        # Side-channel for GUI event-marker rendering (viz/world_builder.py's
        # update_event_marker()): observing this must never affect
        # EventQueue's own pop-based consumption, which nav/algorithm.py's
        # tick() owns exclusively.
