...
PyBullet-backed providers that nav/algorithm.py polls synchronously
...
in nav/algorithm.py needs near-zero changes at the substitution sites.
"""
...
class EventQueue:
    """Multi-slot mailbox for newly-armed events: push() appends (capped at
    max_pending), pop_new() atomically drains everything since the last
    call in arrival order, so a same-tick burst isn't clobbered down to one
    event. A "__RESET__" kind clears the queue without being delivered."""
