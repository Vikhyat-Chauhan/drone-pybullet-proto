...
def run_analysis(zone_metric: str = "mean", cfg: Optional["TeleopConfig"] = None) -> Dict[str, Any]:
    """
    ...
    Required columns (case-insensitive):
      strategy | elapse_time | zone_violations
    ...
    Physically-grounded propulsion/compute power+energy (see
    analysis/power_estimate.py) is included if compute_energy_j and
    propulsion_energy_j are both present -- this is the pipeline's sole
    propulsion energy figure (see docs/POWER_MODEL.md §2 for why the older
    EPM literature constant isn't surfaced instead).

    `cfg` should be the SAME TeleopConfig instance the experiment run was
    made with -- results_csv_path/analyzer_out_dir may have been overridden
    (e.g. via `run.py`'s Hydra entrypoint), and a fresh default TeleopConfig()
    would silently read/write the wrong paths. Defaults to a fresh
    TeleopConfig() only for standalone/manual invocation (e.g. `make analyze`).
    """
    ...
    # Physically-grounded power/energy metrics (see power_estimate.py).
    # Optional: only computed if source columns are present, so this doesn't
    # break analysis of older/partial CSVs.
    _phys_cols = ("compute_energy_j", "propulsion_energy_j")
    _have_phys_cols = all(
    ...
    if _have_phys_cols:
        ...
        df_phys = per_run_physical_metrics(df_raw)
    ...
    if df_phys is not None:
        ...
        data["_propulsion_power_w"] = df_phys["propulsion_power_w"].reindex(df_raw.index)
        data["_energy_per_mission_kj"] = df_phys["energy_per_mission_kj"].reindex(df_raw.index)
    else:
        for key in ("_compute_power_w", "_propulsion_power_w", "_energy_per_mission_kj"):
    ...
    summary = pd.DataFrame({
        ...
        # Zone violations are near-always 0 by design (the drone-building
        # collision filter makes a nav miss a soft zone_violation, not a
        # bounce) -- median adds nothing beyond mean in that common case,
        # so only mean is kept.
    ...
    # Accumulated compute energy per run (Joules, not power) -- dominated by
    # mission duration, so it tracks time-to-goal rather than per-event
    # compute intensity (compute_latency_*_us isolates that instead). E.g.
    # CA has 11x APE1's per-event latency but ~42% less total compute energy
    # per run, since it finishes sooner. Only mean kept: _median tracked it
    # too closely to be worth a separate column.
    ...
    # Physically-grounded power/energy summary (see power_estimate.py for the
    # model). total_power_w and endurance_min aren't surfaced here: compute
    # power is ~4-5 orders of magnitude smaller than propulsion power for
    # this airframe, so total_power_w reads identically to
    # propulsion_power_w_mean and endurance_min barely varies by strategy.
    if not df_f["_propulsion_power_w"].isna().all():
        summary["compute_power_w_mean"]      = g["_compute_power_w"].mean()
        summary["propulsion_power_w_mean"]   = g["_propulsion_power_w"].mean()
