...
ported from CANavigator's arena_generator_city.py / arena_generator_perlin.py.
Self-contained -- no dependency on the CANavigator package. Unlike the
originals, this never writes a Gazebo SDF: rectangles are plain Building
dataclasses, and write_meta() emits only the JSON meta format
violations.py::load_rects() consumes.

As in the original (ArenaGenCfg(pass_through=True, visual_alpha=0.0) at
CANavigator's main.py call site), these rectangles are a horizontal
no-fly-zone *scoring overlay*, not physical obstacles -- main.py does not
give them a PyBullet collision or visual shape.
"""
...
# Legacy flat NFZ slab height, matching CANavigator's Z_THICK/Z_EPS: the
# original generator never varied building height -- every rectangle got
# the same uniform z-extent, since the rectangles are a horizontal
# no-fly-zone overlay, not literal building models. See "buildings are
# pass-through, not physical obstacles" note on generate_buildings().
LEGACY_Z_SIZE = 5.0
LEGACY_Z_CENTER = 0.0
...
@dataclass
class CityLayout:
    """Extends generate_buildings()'s output with purely cosmetic ground
    detail for the PyBullet renderer -- roads (the grid line strips),
    pavement (the developed footprint of blocks that got at least one
    building), and grass (lots that didn't get filled, i.e. the
    original's "leave as courtyard/park/parking (free)" case). None of
    this feeds violations.py or the LiDAR/collision setup -- only
    `.buildings` does, exactly as before."""
...
def generate_city_layout(cfg: ArenaCfg) -> CityLayout:
    """Ported verbatim from _NoFlyCity.run() for the building placement
    (road lines carve the domain into blocks, blocks are inset by a
    road-half-width + sidewalk setback, each block is tiled into lots,
    each lot independently filled with probability fill_prob), extended
    to also record road/pavement/grass rectangles for rendering.

    IMPORTANT: in CANavigator the *buildings* are a horizontal no-fly-zone
    *overlay* used only for violations.py's zone_violations metric --
    the original always generates them pass_through=True, visual_alpha=0.0
    (invisible, non-collidable; see ca_navigator/main.py's ArenaGenCfg(...)
    call). They are NOT physical LiDAR obstacles. main.py mirrors that: it
    writes these rectangles to nofly meta for violation tracking but only
    gives `.buildings` a PyBullet collision shape (see build_arena_bodies'
    docstring for why they still get one despite pass_through=True). The
    road/pavement/grass rects here are cosmetic-only and never get a
    collision shape at all.
    """
    ...
    for (xa, xb) in x_blocks:
        for (ya, yb) in y_blocks:
            ...
            if block_developed:
                # Approximates "driveways/sidewalks near buildings" as the
                # whole inset lot footprint (xa_i..xb_i, ya_i..yb_i --
                # strictly inside the road+setback margin, so it never
                # overlaps road_rects) rather than routing individual
                # per-lot connector paths -- a block-granularity pavement
                # pad, not a literal driveway per building.
...
def generate_buildings(cfg: ArenaCfg) -> list[Building]:
    ...
    road/pavement/grass ground-detail layout used by main.py's renderer."""
